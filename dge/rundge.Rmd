...
# ----- Glycolysis & gluconeogenesis heatmap -----
glycgluc <- genesets2[["KEGG_GLYCOLYSIS_GLUCONEOGENESIS"]]
temp <- c()
for(i in 1:length(glycgluc)){temp <- rbind(temp,filter(hm3, geneID==glycgluc[i]))}
glycgluc <- c(t(unite(temp,"ENSSHAG_geneID", ENSSHAG:geneID, sep=" ")))

glycgluc2 <- subset(mx.scaled, rownames(mx.scaled) %in% glycgluc)
glycglucFC <- subset(log2FC, rownames(log2FC) %in% glycgluc)
glycglucAE <- subset(mean, rownames(mean) %in% glycgluc)

h4 <- Heatmap(glycgluc2, cluster_rows = F,
              column_labels = colnames(glycgluc2), name="Z-score",
              cluster_columns = T)
h5 <- Heatmap(glycglucFC, row_labels = rownames(glycglucFC), 
            cluster_rows = F, name="logFC", top_annotation = ha, col = col_logFC,
            cell_fun = function(j, i, x, y, w, h, col) { # add text to each grid
              grid.text(round(as.numeric(glycglucFC[i, j],2)), x, y)
            })
h6 <- Heatmap(glycglucAE, row_labels = rownames(glycglucAE), 
            cluster_rows = F, name = "AveExpr", col=col_AveExpr,
            cell_fun = function(j, i, x, y, w, h, col) { # add text to each grid
              grid.text(round(as.numeric(glycglucAE[i, j],2)), x, y)},
            column_title = "KEGG_GLYCOLYSIS_GLUCONEOGENESIS")
...
pathview(gene.data=pathFC, pathway.id="hsa00010", species="hsa",gene.idtype="SYMBOL", node.sum="sum",
         limit=list(gene=c(-10,10), cpd=1), bins=list(gene=20, cpd=1),
         low="blue",mid="white",high="red",
         same.layer=FALSE)
...
ox1 <- genesets2[["KEGG_OXIDATIVE_PHOSPHORYLATION"]]
ox2 <- genesets2[["GOBP_OXIDATIVE_PHOSPHORYLATION"]]
ox3 <- genesets2[["WP_OXIDATIVE_PHOSPHORYLATION"]]
ox <- list(ox1=ox1,ox2=ox2,ox3=ox3)
title <- c("KEGG_OXIDATIVE_PHOSPHORYLATION","GOBP_OXIDATIVE_PHOSPHORYLATION","WP_OXIDATIVE_PHOSPHORYLATION")

for(i in 1:length(ox)){
  temp <- c()
  for(j in 1:length(ox[[i]])){temp <- rbind(temp,filter(hm3, geneID==ox[[i]][j]))}
  oxphos <- c(t(unite(temp,"ENSSHAG_geneID", ENSSHAG:geneID, sep=" ")))
  oxp <- subset(mx.scaled, rownames(mx.scaled) %in% oxphos)
  oxpFC <- subset(log2FC, rownames(log2FC) %in% oxphos)
  oxpAE <- subset(mean, rownames(mean) %in% oxphos)
  
  h1 <- Heatmap(oxp, cluster_rows = F,
                column_labels = colnames(oxp), name="Z-score",
                cluster_columns = T)
  h2 <- Heatmap(oxpFC, row_labels = rownames(oxpFC), row_names_gp = gpar(fontsize = 8),
              cluster_rows = F, name="logFC", top_annotation = ha, col = col_logFC,
              cell_fun = function(j, i, x, y, w, h, col) { # add text to each grid
                grid.text(round(as.numeric(oxpFC[i, j],2)), x, y, gp=gpar(fontsize=8))
              })
  h3 <- Heatmap(oxpAE, row_labels = rownames(oxpAE), row_names_gp = gpar(fontsize = 8),
              cluster_rows = F, name = "AveExpr", col=col_AveExpr,
              cell_fun = function(j, i, x, y, w, h, col) { # add text to each grid
                grid.text(round(as.numeric(oxpAE[i, j],2)), x, y, gp=gpar(fontsize=8))},
              column_title=title[i])
...
```{r, pathview2}
# there is a reported bug for hsa01212 in pathview: https://github.com/datapplab/pathview/issues/28
# hence will have to plot other pathways, eg "fatty acid degradation", instead of "fatty acid metabolism"

pathview(gene.data=pathFC, pathway.id="hsa00061", species="hsa",gene.idtype="SYMBOL", node.sum="sum",
         limit=list(gene=c(-10,10)), bins=list(gene=20),
         low="blue",mid="white",high="red",
         same.layer=F)

pathview(gene.data=pathFC, pathway.id="hsa00062", species="hsa",gene.idtype="SYMBOL", node.sum="sum",
         limit=list(gene=c(-10,10)), bins=list(gene=20),
         low="blue",mid="white",high="red",
         same.layer=F)

pathview(gene.data=pathFC, pathway.id="hsa00071", species="hsa",gene.idtype="SYMBOL", node.sum="sum",
         limit=list(gene=c(-10,10)), bins=list(gene=20),
         low="blue",mid="white",high="red",
         same.layer=F)

pathview(gene.data=pathFC, pathway.id="hsa00190", species="hsa",gene.idtype="SYMBOL", node.sum="sum",
         limit=list(gene=c(-10,10), cpd=1), bins=list(gene=20, cpd=1),
         low="blue",mid="white",high="red",
         same.layer=F)
