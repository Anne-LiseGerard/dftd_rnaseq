...
h1 <- Heatmap(met40, cluster_rows = F,
              ...
              cluster_columns = T)
h2 <- Heatmap(met40FC, row_labels = rownames(met40FC), 
              cluster_rows = F, name="logFC", top_annotation = ha, col = col_logFC,
              cell_fun = function(j, i, x, y, w, h, col) { # add text to each grid
                grid.text(round(as.numeric(met40FC[i, j],2)), x, y)
              })
h3 <- Heatmap(met40AE, row_labels = rownames(met40AE), 
              cluster_rows = F, name = "AveExpr", col=col_AveExpr,
              cell_fun = function(j, i, x, y, w, h, col) { # add text to each grid
                grid.text(round(as.numeric(met40AE[i, j],2)), x, y)},
