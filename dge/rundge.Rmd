...
# merge cholesterol pathways
cholesterol1 <- genesets2[["WP_CHOLESTEROL_BIOSYNTHESIS_PATHWAY"]]
cholesterol2 <- genesets2[["REACTOME_CHOLESTEROL_BIOSYNTHESIS"]]
cholesterol3 <- genesets2[["WP_CHOLESTEROL_METABOLISM"]]
cholesterol4 <- genesets2[["HALLMARK_CHOLESTEROL_HOMEOSTASIS"]]
cholesterolx <- unique(c(cholesterol1,cholesterol2,cholesterol3,cholesterol4))

glycolysis <- genesets2[["WP_AEROBIC_GLYCOLYSIS"]]


mx <- dge[,7:ncol(dge)]
mx$gene <- row.names(mx) 
mx <- separate(mx, gene, into = c("ENSSHAG", "geneID"), sep = " (?=[^ ]+$)")
mx_chol <- data.frame()
mx_glyc <- data.frame()
for(i in 1:length(cholesterolx)){
  temp <- filter(mx, geneID==cholesterolx[i])
  mx_chol <- rbind(mx_chol, temp)
}
for(i in 1:length(glycolysis)){
  temp <- filter(mx, geneID==glycolysis[i])
  mx_glyc <- rbind(mx_glyc, temp)
}
rownames(mx_chol) <- mx_chol$geneID
mx_chol <- mx_chol[,1:6]
rownames(mx_glyc) <- mx_glyc$geneID
mx_glyc <- mx_glyc[,1:6]

heatmap.2(as.matrix(mx_chol),trace="none",scale="row",
          col=colfunc(25),ColSideColors=cols[1:6],cexRow=0.4,cexCol=0.75,
          density.info=c("none"), srtCol=0, key.title=NA,
          main='cholesterol metabolism')
heatmap.2(as.matrix(mx_glyc),trace="none",scale="row",
          col=colfunc(25),ColSideColors=cols[1:6],cexRow=0.4,cexCol=0.75,
          density.info=c("none"), srtCol=0, key.title=NA,
          main='aerobic glycolysis')
